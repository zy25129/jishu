"""
AEM反演模型预测与评估脚本
直接从 data/inverse/ 加载数据，从 models/ 加载模型
每个模型的结果保存到 results/inverse/<MODEL>/
"""
import os
import sys
import argparse
...
def evaluate(device):
    ...
    scaler_y = joblib.load('models/scaler_y_inverse.pkl')
    scaler_X = joblib.load('models/scaler_X_inverse.pkl')
    heights_test = X_test[:, -1]

    all_results = {}

    for mtype in MODEL_TYPES:
        model_path = f'models/inverse_{mtype}_best.pth'
        ...
        print(f"\n评估 {name}...")
        model = AEMInverseModel(model_type=mtype, input_dim=X_train.shape[1],
        ...
        print_metrics(metrics, f"Inverse {name}")

        # 各模型独立图表
        plot_true_vs_predicted(targets, preds, model_name=f'Inverse-{name}', n_samples=3,
                               save_path=f'{model_dir}/profile_overlay.png')
        plot_error_distribution(targets, preds, model_name=f'Inverse-{name}',
                                save_path=f'{model_dir}/error_dist.png')
        analyze_by_layer(targets, preds, model_name=f'Inverse-{name}',
        ...
        analyze_by_height(targets, preds, heights_orig, model_name=f'Inverse-{name}',
...
def predict(device):
    ...
    for mtype in MODEL_TYPES:
        ...
        print(f"\n使用 {name} 预测...")
        model = AEMInverseModel(model_type=mtype, input_dim=X_predict.shape[1],
        ...
        create_submission(preds, f'{DATA_DIR}/predict_source.csv',
                          output_path=f'{model_dir}/submission.csv', mode='inverse')
