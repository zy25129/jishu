"""
AEM正演模型训练脚本
直接从 data/forward/ 加载预处理好的数据
结果保存到 results/forward/
"""
import os
import sys
import argparse
...
def train_single_model(model_type, X_train, y_train, X_val, y_val,
    ...
    model_path = f'models/forward_{model_type}_best.pth'
    ...
    print_metrics(final_metrics, f"Forward {model_type.upper()}")
    return history, final_metrics


def main():
    parser = argparse.ArgumentParser(description='AEM正演模型训练')
    ...
    configs = [('lstm', 'LSTM编码器-解码器+注意力+残差 (主模型)'),
               ('gru', 'GRU对比模型')]
